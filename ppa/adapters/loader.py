...
class AdapterFrontend:
    ...
    css: Optional[str] = None  # relative path to a CSS file in the adapter's static dir
    js: Optional[str] = None  # relative path to a JS file in the adapter's static dir
    stimulus_controllers: Optional[List[str]] = None  # controller names to register
...
def _resolve_adapter_path(path_or_name: str) -> Path:
    # If absolute path or contains a slash, treat as filesystem path.
    p = Path(path_or_name)
    if p.exists() and p.is_dir():
        return p
    # Otherwise treat as name under ADAPTERS_DIR
...
def get_adapter(adapter_name: Optional[str] = None) -> Optional[Adapter]:
    ...
    Args:
        adapter_name: Name of adapter to load, or None for global adapter
    ...
    # If no name specified, use global adapter from settings
...
def get_adapters_for_work(work) -> List[Adapter]:
    ...
    Args:
        work: DigitizedWork instance

    Returns:
        List of Adapter instances (may be empty)
    """
    adapters = []
    adapter_names = set()

    # Get adapter names from all collections
    for collection in work.collections.all():
        if collection.adapter_name:
            adapter_names.add(collection.adapter_name)

    # If no collection-specific adapters, use global adapter
...
def get_primary_adapter_for_work(work) -> Optional[Adapter]:
    """
    Get the primary adapter for a work.

    For works with multiple collections using different adapters,
    returns the first adapter found. For single-collection works,
    returns that collection's adapter.
