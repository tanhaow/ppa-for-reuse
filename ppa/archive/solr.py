...
class ArchiveSearchQuerySet(AliasedSolrQuerySet):
    ...
    # boosting)
    _title_search = "{!type=edismax qf=$search_title_qf " + "pf=$search_title_pf v=$title_query}"
    ...
    def __init__(self, solr=None):
        ...
        try:
            ...
            # Get all adapter names from ADAPTERS_DIR
            adapters_dir = getattr(settings, "ADAPTERS_DIR", None)
            adapter_names = set()
    ...
    def work_filter(self, *args, **kwargs):
        ...
        kwargs = dict((opt, val) for opt, val in kwargs.items() if val not in [None, ""])
        if args or kwargs:
    ...
    def query_opts(self):
        ...
        keyword_query = (
            "((%s) OR ({!join from=group_id_s to=id v=$content_query}))" % self._keyword_search
        )
        ...
        if self._workq.filter_qs:
            ...
            combined_query = "(%s) AND (%s OR {!join from=id to=group_id_s v=$work_query})" % (
                keyword_query,
                work_query,
            )
