...
if command -v python &> /dev/null; then
    ADAPTER_CHECK=$(python manage.py adapter_validate 2>&1)
    if echo "$ADAPTER_CHECK" | grep -q "successfully"; then
        ADAPTER_NAME=$(echo "$ADAPTER_CHECK" | grep -oE "'[^']+'" | head -1 | tr -d "'")
        echo "✅ Adapter: $ADAPTER_NAME validated"
    elif echo "$ADAPTER_CHECK" | grep -q "not configured"; then
